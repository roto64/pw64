...
function build_all () {
  ...
  # Converted the memory copy code to C (for whatevers).
  mips64-elf-gcc -Wall -O3 -mtune=vr4300 -march=vr4300 -mabi=32 -fomit-frame-pointer -G0 -c memload.c -o memload.o
  ...
  mips64-elf-objcopy -j .text -O binary memload.o memload.bin
  ...
  mips64-elf-objdump -b binary -m mips:4300 -D -EB memload.bin
  #####
...
function clean_all () {
  ...
  rm injection_test.bin injection_test.o hook.o memload.o memload.bin
}
